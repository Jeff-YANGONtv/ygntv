...
import { AlertCircle, CheckCircle2, Clock3, Crown, LogOut, ReceiptText, ShieldCheck, UserRound } from 'lucide-react';
...
export function ProfilePage() {
  ...
  useEffect(() => {
    if (!token) return;
    let mounted = true;
    Promise.all([getTvProfile(), getPaymentOrders()])
    ...
    {loading ? <div className="profile-loading" role="status">Loading your profile…</div> : <>
      <div className="profile-grid">
        <article className="profile-identity profile-card">
          ...
          <div className="profile-identity-copy"><h2>{displayName}</h2><p>{email}</p><span className="profile-role"><ShieldCheck size={13} /> {profile?.user.role ? profile.user.role : 'Account'}</span></div>
        </article>
