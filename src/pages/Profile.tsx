    ...
    {loading ? <div className="profile-loading" role="status">Loading your profile…</div> : <>
      <div className="profile-grid">
        <article className="profile-identity profile-card">
          ...
          <div className="profile-identity-copy"><h2>{displayName} {profile?.user.uid && <span className="profile-uid">UID {profile.user.uid}</span>} <span className={isPremium ? 'profile-badge profile-badge--premium' : 'profile-badge'}><BadgeCheck size={14} /> {isPremium ? 'Premium' : 'No membership'}</span></h2><p>{email}</p><span className="profile-role"><ShieldCheck size={13} /> {profile?.user.role ? profile.user.role : 'Account'}</span></div>
        </article>
        <article className={`profile-card premium-card${isPremium ? ' premium-card--active' : ''}`}>
          <div className="premium-icon" aria-label="Membership lifetime"><Clock3 size={24} /></div>
          <div><span className="profile-card-label">Membership lifetime</span><h2>{isPremium ? term : 'No active membership'}</h2><p>{isPremium ? (entitlement?.valid_until ? `Valid until ${formatDate(entitlement.valid_until)}` : 'Lifetime access is active.') : 'Join Premium to watch and download videos.'}</p></div>
          {isPremium && <span className="premium-state premium-state--active">Lifetime</span>}
        </article>
      </div>
