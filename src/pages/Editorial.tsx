...
export function BlogPage() {
  ...
  useEffect(() => { load(); }, []);
  return <div className="page page-blog"><section className="container page-heading page-heading--wide"><div><h1>Blog</h1></div></section><section className="container blog-toolbar"><SearchField value={query} onChange={setQuery} onSubmit={() => load(query)} placeholder="Search blog posts..." /></section><section className="container blog-list">{loading ? <SkeletonGrid count={3} /> : error ? <ErrorState onRetry={() => load(query)} /> : posts.length === 0 ? <EmptyState title="No stories found" copy="Try another phrase or check back soon." /> : <>{posts.map((post, index) => <Link to={blogPath(post)} className={`article-row ${index === 0 ? 'article-row--featured' : ''}`} key={post.id}><img src={mediaUrl(post.cover || post.image)} alt={post.cover_alt || post.title} loading="lazy" decoding="async" width="1280" height="720" /><div className="article-row__copy"><span className="eyebrow">{post.topic || post.category} · {post.date}</span><h2>{post.title}</h2><p>{post.excerpt}</p><span className="text-link">Read story <ArrowRight size={15} /></span></div><span className="article-number">0{index + 1}</span></Link>)}</>}</section></div>;
}
