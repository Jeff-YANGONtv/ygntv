...
import { getMediaBySlug, getMovies, getSeries, getTvPlayback, mediaPlaybackUrl, mediaUrl, saveTvViewingProgress } from '../lib/api';
...
const sourceUrl = mediaUrl(source);
  ...
  const resolvedSource = mediaPlaybackUrl(directMediaUrl(sourceUrl));
...
function PremiumGate({ onJoin }: { onJoin: () => void }) {
  return <div className="premium-gate" role="dialog" aria-label="Premium membership required">
    <span className="eyebrow">Premium access</span>
    <h2>Premium membership required</h2>
    <p>Join Premium to watch and download this video.</p>
    <button className="button button--primary" type="button" onClick={onJoin}>View Premium membership</button>
  </div>;
}
