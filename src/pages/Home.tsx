...
export function Home() {
  ...
  return <div className="page page-home page-home--sliders">
    ...
    {movies.length > 0 && <AutoSlider className="container home-banner-slider" eyebrow="Trending now" title="Trending Movies" action={{ label: 'View all movies', to: '/movies' }}>{movies.slice(0, 8).map((item) => <MediaBanner item={item} key={item.id} onWatch={() => watch(item)} />)}</AutoSlider>}
    {series.length > 0 && <AutoSlider className="container home-banner-slider" eyebrow="Binge-worthy stories" title="Series Movies" action={{ label: 'View all series', to: '/series' }}>{series.slice(0, 8).map((item) => <MediaBanner item={item} key={item.id} onWatch={() => watch(item)} />)}</AutoSlider>}
