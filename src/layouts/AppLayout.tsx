...
import { Bell, CheckCheck, Clapperboard, Crown, Film, History, Home, Info, LogOut , Menu, Newspaper, Send, UserRound, X } from 'lucide-react';
...
const navigation: { label: string; to: string; icon: typeof Home }[] = [
...
const aboutNavigationItem = { label: 'About Us', to: '/about', icon: Info };
...
export function AppLayout() {
  ...
  return <div className="app-shell">
    <header className="site-header">
      {!location.pathname.startsWith('/about') && marqueeText.length > 0 && <div className="header-marquee" aria-label="Latest Yangon TV announcements"><div className="container header-marquee__viewport"><div className="header-marquee__track"><span>{marqueeText.map((text) => `✦ ${text}`).join('     ')}</span><span aria-hidden="true">{marqueeText.map((text) => `✦ ${text}`).join('     ')}</span></div></div></div>}
...
function AboutMarquee() {
  const text = 'Yangon TV သည် နိုင်ငံတကာ ရုပ်ရှင်ကားကြီးများ၊ နာမည်ကြီး ဇာတ်လမ်းတွဲများနှင့် ရုပ်သံဖျော်ဖြေရေးအစီအစဉ်များကို မြန်မာစာတန်းထိုးဖြင့် ကြည်လင်ပြတ်သားစွာ တစ်နေရာတည်းမှာ ကြည့်ရှုနိုင်စေတဲ့ Online Streaming Platform ဖြစ်ပါတယ်။';
